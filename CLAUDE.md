...
- **commands.rs** — `Command` (`BotCommands`): `/help`, `/events`, `/today`, `/tomorrow`, `/week`, `/month`, admin `/import <user_id>`/`/database`/`/logs`, hidden `/exit`. List commands paginate via `ListKind`; `handle_list`/`handle_list_callback` edit in place with `list_keyboard` (`◀ Prev` / `<page>/<total>` no-op indicator / `Next ▶`). `ListKind::row_style()` picks the per-row layout: `/events` → two-line, the hidden `Missed` kind (`ms` tag, used only by the startup missed-events send + its page-turn callbacks — see state.rs; `format_missed_page` builds page 0) → preview-only `/event<id>` rows, the rest → single-line. **Single-event view** `/event<id>` (matched manually in `main`, not in the command menu): `handle_event_view` → `event_detail` + `event_actions_keyboard` (Dismiss/Dismiss-repetition/Edit/Delete; `⏭ Dismiss` shows only for active events, `⏩ Dismiss repetition` only when the event is active **and** its current `source` is `Repetition`). Event callbacks decoded by `parse_event_callback`, dispatched by `handle_event_callback`: `dis` dismiss, `disr` dismiss-repetition, `sn` snooze, `del`/`delyes`/`delno` delete, `ed`/`edno` edit. Dismiss delegates to `EventProvider::dismiss` (advances past the current occurrence — see state.rs); dismiss-repetition delegates to `EventProvider::dismiss_repetition` (skips the interval fills to the next non-`Repetition` anchor — see state.rs); both re-render the detail view in place. Snooze inserts a one-off `snoozed` event reusing the original's HTML message. Delete/edit prompts edit the keyboard/message in place. All id-bearing handlers access-check.
