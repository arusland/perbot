...
- **main.rs** — Bot entry point. Initializes teloxide REPL. Uses a **top-events scheduling model**: on startup loads the 10 nearest active events via `storage.get_top_events(now)` into an in-memory `TopEvents` struct (holds a sorted `Vec<EventInfo>` and an `AbortHandle` for the current scheduled task), then schedules only the first event via `schedule_first_event`. Handles incoming messages: for every message (any type), chat info is saved/updated via `storage.upsert_chat`. For text messages, the message is stored via `storage.insert_message` (capturing user_id and chat_id) to obtain a `msg_id`. Then parses with `parser::parse`, sets `chat_id` and `msg_id` directly on the `EventInfo`, computes datetime with `scheduler::calc_next`, saves via `storage.insert_event`, then calls `reload_and_schedule` which reloads top events from DB and reschedules the first (aborting the previous task if needed). When the first event fires: sends the message, calls `calc_next_at` to compute the next occurrence, saves via `update_schedule`, removes the event from the in-memory list, re-inserts it sorted if still active, reloads from DB if the list empties, and schedules the next first event. All responses use MarkdownV2 parse mode (escaped via `escape_markdown`). Storage is shared as `Arc<Mutex<EventStorage>>`, top events state as `Arc<Mutex<TopEvents>>`.
