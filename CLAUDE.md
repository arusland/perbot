...
- **state.rs** — `EventProvider`: `Clone` handle over `Arc<Mutex<EventProviderState>>` (storage + cached `next_event`). `start(msg_tx)` sends missed events (no keyboard) then spawns a 1s poll thread firing due events (each built here with the `SNOOZE_HINT` appended and `snooze_keyboard(id)` in `reply_markup`) and rescheduling. Snooze presentation (`SNOOZE_OPTIONS`/`SNOOZE_HINT`/`snooze_keyboard`, callback `eid:<id>:sn:<minutes>`: 1/5/10/30 min, 1/2/8 h, 1 day) lives here, next to the only code that uses it. Wraps storage plus `insert_event_and_get[_at]`, `insert_prebuilt_event` (insert already-scheduled event without re-scheduling; used by importer and snooze), `update_at_and_reload`.
