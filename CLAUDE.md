...
- **telegram.rs** — All output is HTML (`teloxide::utils::html::escape` for user/bot text; `event.message` embedded verbatim as an HTML fragment). `format_when(now, dt)` → plain-text `HH:MM dd.mm.yyyy (relative)` for a single datetime; `next_launches_preview(event, after)` walks `scheduler::calc_next_at` forward from `after` to list up to `MAX_NEXT_PREVIEW` (3) upcoming launches as `• <format_when>` bullets, plus a trailing `• ...` when more remain (empty string for one-off events); returns plain text (callers `html::escape` it, a no-op for these chars). `scheduled_message(now, dt, event)` → shared `Scheduled message for <b><format_when></b>` confirmation (bolded absolute datetime + relative time from `now`, e.g. `13:30 22.06.2026 (1d)`; used on new parse in `main.rs` and on snooze) followed by a `Message: <event.message>` line (HTML fragment), with the `next_launches_preview` appended for recurring events; `format_page[_at](..., two_line)` → `(text, total_pages)` HTML paginated lists (title `<b>…(page x/y):</b>`, `(page x/y)` only when >1 page). The `two_line` flag picks the row layout: `false` → single-line `• <when> — <message HTML>` (`write_event_row`); `true` (only `/events`) → a two-line row (`write_event_row_two_line`): a bold `• <when>` line — with `, <recurrence>` appended for repeating events (`describe_recurrence`, e.g. `every 2 days`/`every friday`/`every first sunday`/`last day of the month`) — then an indented **plain-text** message preview (`message_preview`, `MESSAGE_PREVIEW_MAX` = 50). `describe_recurrence(event)` renders the event's recurrence (mutually-exclusive `repetition`/`days`/`monthly_pattern`, checked in that order) as plain text, `None` for one-off events; helpers `weekday_name`/`ordinal_name` plus `TimeUnit::label(plural)`. `message_preview(html_fragment, max)` derives a short plain preview from the stored HTML fragment: strips tags, unescapes the three `html::escape` specials (`&amp; &lt; &gt;`), collapses all whitespace (incl. newlines) to single spaces, and truncates by **char count** to `max` with a trailing `...` (callers re-escape it for HTML output). Helpers `LIST_PAGE_SIZE`, `total_pages`. `extract_chat_info(chat)` → `ChatInfo`.
