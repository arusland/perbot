...
- **commands/** — One module per command (or group of similar commands). `mod.rs` holds `Command` (`BotCommands`: `/help`, `/events`, `/today`, `/tomorrow`, `/week`, `/month`, admin `/import <user_id>`/`/database`/`/logs`, hidden `/exit`), the shared `CmdContext`, the `Command::handle` dispatch, and `pub use` re-exports so external callers keep the flat `commands::X` paths.
  - **list.rs** — the paginated list commands. `ListKind` describes each list; `handle_list`/`handle_list_callback` edit in place with `list_keyboard` (`◀ Prev` / `<page>/<total>` no-op indicator / `Next ▶`). `ListKind::row_style()` picks the per-row layout: `/events` → two-line, the hidden `Missed` kind (`ms` tag, used only by the startup missed-events send + its page-turn callbacks — see state.rs; `format_missed_page` builds page 0) → preview-only `/event<id>` rows, the rest → single-line.
