...
- **types.rs** — `EventInfo` (parsed + DB fields), `MessageInfo`, `ChatInfo`/`ChatType`, `TgMessage`, `MessageSender`, `PageRequest` (`page`/`size` + `offset()`) and `PageResponse` (`events`/`total`) — the paging argument/result of the `EventProvider` list methods, time enums (`TimeUnit`/`Repetition`/`Ordinal`/`MonthlyPattern`), and `NextSource` (which field produced `next_datetime`; see EventInfo `source`). **Fixed canonical (storage) helpers** live only here: `day_to_str`, `parse_days`, `unit_from_str`, `TimeUnit::label`, `NextSource::as_str`/`from_str` (locale-independent, reused by the English locale). `EventInfo::normalize_time(loc)` (canonical re-parseable string — see Invariants) pulls all its vocabulary from `loc`; user-facing name helpers (`weekday_full`/`ordinal_word`/`ordinal_suffix`/`weekday_abbrev_cap`) now live in `locale`.
...
- **storage.rs** — `EventStorage` over rusqlite. Tables `chats`, `messages`, `events` (`events.msg_id` NOT NULL FK; `legacy`/`snoozed` flags; `last_next_datetime` tracks the last non-null `next_datetime`; `source` TEXT holds `NextSource::as_str()`), plus the **`missed_events` helper table** (autoincrement `id` preserving missed order + `event_id` UNIQUE FK `ON DELETE CASCADE`, so deleting an event drops it from the missed list) — the startup missed snapshot, cleared and repopulated on every start. Standard CRUD + range/active/missed queries + `backup_to` (`VACUUM INTO`). **List queries page in SQL**: `get_active_by_chat[_on_date/_in_range]` take `limit`/`offset` (`LIMIT ?/OFFSET ?`) and pair with `count_active_by_chat[_on_date/_in_range]` for page-count math, so a chat's full list is never loaded whole; `get_missed_events(now, limit)` batches the missed backlog the same way (no offset — rescheduling a batch removes it from the predicate). Missed-snapshot access: `clear_missed_events`, `insert_missed_events(&[ids])`, `get_missed_chat_ids`, and `count_missed_snapshot_by_chat`/`get_missed_snapshot_by_chat(chat_id, limit, offset)` (join to `events`, ordered by insertion). `update_schedule(id, active, next_datetime, last_next_datetime, source)` persists a reschedule (including the new source).

- **state.rs** — `EventProvider`: `Clone` handle over `Arc<Mutex<_>>` (storage + cached `next_event`). **Storage-backed methods propagate `Result`** — the read/mutation wrappers (`get_event`, `get_active_by_chat[_on_date/_in_range]`, `move_missed_events`, `get_missed_snapshot_events`, the insert/update/`delete`/`update_at_and_reload` wrappers, private `get_events_at`/`load_next_event`) return `crate::error::Result` and `?`-bubble DB errors instead of logging-and-defaulting; only `get_next_event` (in-memory cache) is infallible. **List reads are paged**: `get_active_by_chat[_on_date/_in_range](…, PageRequest)` and `get_missed_snapshot_events(chat_id, PageRequest)` return a `PageResponse` — one page of rows plus the list's total — with count + page fetched under one lock (both page via SQL `LIMIT`/`OFFSET`; the missed snapshot reads the `missed_events` table joined to `events`). `start(msg_tx) -> Result<()>` sends missed events then spawns a 1s poll thread that fires due events and reschedules — a **startup** DB error (missed-events move/reschedule) returns `Err` before the thread spawns so `main` can notify the admin and not start the bot; once the detached poll thread is running its per-tick errors are logged in place (no caller to return to). **Missed events** render as a paged, preview-only `/event<id>` list (`commands::format_missed_page`): `start` clears the `missed_events` table, then `move_missed_events` records + reschedules the missed backlog in `MISSED_MOVE_BATCH`-sized batches (never whole in memory — each rescheduled batch leaves the missed predicate, so the next `get_missed_events(now, limit)` fetch is the next batch), then each chat from `get_missed_chat_ids` is sent page 0 read back via `get_missed_snapshot_events(chat_id, PageRequest)` — the same call backing the `ms:<page>` page-turn callbacks. Fired text = `<message><preview>\n\n<SNOOZE_HINT>` (message + preview are HTML fragments, hint escaped). **Snooze presentation lives here** (`SNOOZE_OPTIONS`/`SNOOZE_HINT`/`snooze_keyboard`, callback `eid:<id>:sn:<minutes>`). Insert/update wrappers: `insert_event_and_get[_at]`, `insert_prebuilt_event` (no re-scheduling; importer + snooze), `update_event_and_get[_at]` (edit flow), `delete` (reloads cached next), `dismiss(id, chat_id) -> DismissOutcome` (the `/event<id>` Dismiss action — access-checks, reschedules at `next_datetime + 1s`, reloads cached next; returns the updated event / `Inactive` / `NotFound`), `dismiss_repetition(id, chat_id) -> DismissOutcome` (the `/event<id>` Dismiss-repetition action — advances one step at a time via `calc_next_at(_, prev + 1s)` until the resulting `source` is no longer `Repetition`, i.e. the next anchor, capped at a 100-year horizon; events with no anchor field — no short date or `monthly_pattern` — stay `Repetition` forever, so those skip the loop and fall back to a single ordinary dismiss).

- **commands/** — One module per command (or group of similar commands). `mod.rs` holds `Command` (`BotCommands`: `/help`, `/events`, `/today`, `/tomorrow`, `/week`, `/month`, admin `/import <user_id>`/`/database`/`/logs`, hidden `/exit`), the shared `CmdContext`, the `Command::handle` dispatch, and `pub use` re-exports so external callers keep the flat `commands::X` paths.
  - **list.rs** — the paginated list commands. `ListKind` describes each list; `ListKind::fetch(provider, chat_id, page)` returns one page + the total (paging happens in storage, not in memory); `handle_list`/`handle_list_callback` edit in place with `list_keyboard` (`◀ Prev` / `<page>/<total>` no-op indicator / `Next ▶`); a page-turn to a page that no longer exists (events deleted meanwhile) clamps to the last page and refetches. `ListKind::row_style()` picks the per-row layout: `/events` → two-line, the hidden `Missed` kind (`ms` tag, used only by the startup missed-events send + its page-turn callbacks — see state.rs; `format_missed_page` builds page 0) → preview-only `/event<id>` rows, the rest → single-line.
  - **event.rs** — **single-event view** `/event<id>` (matched manually in `main` via `parse_event_command`, not in the command menu): `handle_event_view` → `event_detail` + `event_actions_keyboard` (Dismiss/Dismiss-repetition/Edit/Delete; `⏭ Dismiss` shows only for active events, `⏩ Dismiss repetition` only when the event is active **and** its current `source` is `Repetition`). Event callbacks decoded by `parse_event_callback`, dispatched by `handle_event_callback`: `dis` dismiss, `disr` dismiss-repetition, `sn` snooze (→ snooze.rs), `del`/`delyes`/`delno` delete, `ed`/`edno` edit. Dismiss delegates to `EventProvider::dismiss` (advances past the current occurrence — see state.rs); dismiss-repetition delegates to `EventProvider::dismiss_repetition` (skips the interval fills to the next non-`Repetition` anchor — see state.rs); both re-render the detail view in place. Delete/edit prompts edit the keyboard/message in place. All id-bearing handlers access-check.
  - **snooze.rs** — the `eid:<id>:sn:<minutes>` buttons: inserts a one-off `snoozed` event reusing the original's HTML message.
  - **cancel.rs** — `handle_cancel_pending`, the `pm:` Cancel button of the time-only flow.
  - **help.rs**, **import.rs** (`handle_import` + `handle_import_zip`), **database.rs**, **logs.rs**, **exit.rs** — one file per remaining command.
...
- **main.rs** — Entry point + teloxide `Dispatcher`. Startup clears stale command scopes then `set_my_commands`; a failing `provider.start(msg_tx)` notifies the admin and returns before the dispatcher is built (bot never starts). `message_handler` + `callback_handler` (routes `eid:`/`pm:`/list) return `anyhow::Result` (so storage `Result`s and teloxide `RequestError`s both `?`-bubble) and are each wrapped by a `*_safe` endpoint (`message_handler_safe`/`callback_handler_safe`) that catches errors, tells the user "Something goes wrong!", and forwards the detail to the admin. Holds the two text-completion flows: **time-only** (`PendingMessage` → render body → schedule) and **edit-completion** (`PendingEdit` → re-parse, copy identity fields, `update_event_and_get`). Sender task is a dumb HTML pump.

- **telegram.rs** — All output HTML; every time-bearing helper takes `loc: &dyn LocaleProvider`. `format_when` (single datetime + relative — both from `loc`), `next_launches_preview` (up to 3 upcoming launches under the `loc` header; "" for one-off), `scheduled_message` (confirmation, used on new parse + snooze), `format_page_at` (paginated lists; renders an **already-fetched page slice** plus the list's `total` — callers page at the storage layer — and a `RowStyle` arg — `SingleLine`/`TwoLine`/`PreviewLink` — picks the per-row layout: two-line for `/events`, preview-only `• <plain preview> /event<id>` for the missed list). `event_when_line` (shared bold line, recurrence appended inside the relative `(…)` via `describe_recurrence`), `event_detail` (`/event<id>` view — an **inactive** event renders a single bold "Event is out of date. Last fired at <`last_next_datetime`>" notice instead of the when-line/launches). The relative-time wording lives in `loc.format_relative`. `html_to_plain`/`message_preview` (strip+truncate by char count, locale-free), `event_source_input`/`edit_prompt` (reconstruct re-parseable input as tap-to-copy `<code>`). **Length clamp:** `rendered_len` (UTF-16 units of the rendered text — how Telegram counts) and `clamp_message` (caps a body to `MESSAGE_MAX_LEN` = `TELEGRAM_MAX_LEN` 4096 − reserve, falling back to escaped plain text) enforce Telegram's per-message limit; `main` applies the clamp to a user's reminder body at ingestion and warns with `pending::MESSAGE_TRUNCATED` when it shortens.
...
`test-cases.md` holds markdown tables driving `tests/table_tests.rs`. Rows alternate `USER` (parse + `insert_event_and_get_at`) and `SYSTEM` (`update_at_and_reload`, assert `next_datetime` or `NONE`). A USER row whose Input starts with `!` is an **action row**, not text to parse: `!Dismiss` / `!Dismiss repetition` (case-insensitive after the `!`) call `EventProvider::dismiss` / `dismiss_repetition` on the current event (columns 4–5 empty; the timestamp is unused — both methods advance from the stored `next_datetime + 1s`), and the following SYSTEM row asserts the result; any other `!command` fails the table. Column 4 is overloaded by actor: on USER rows it is the expected `event.message`; on SYSTEM rows it is the expected source as `source=<name>` (asserted against `event.source.as_str()`; empty on `NONE` rows). Column 5 = expected `normalize_time()` (asserted on USER rows; empty when input doesn't parse). A literal `\n` in Input/Message is decoded to a real newline. Add scenarios by appending `###` sections — no code changes needed.
