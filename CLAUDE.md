...
- **storage.rs** — SQLite persistence via rusqlite. `EventStorage` manages two tables: `events` (id, chat_id, date, time, year_explicit, message, active, next_datetime, created_at, days, repeat_interval, repeat_unit, in_offset, in_offset_unit, bare_hour, monthly_pattern, raw_msg) and `chats` (id, chat_type, title, username, first_name, last_name, updated_at). Provides `open(path)` for file-backed DB and `open_in_memory()` for tests. `insert_event(&StoredEvent)` persists an event. `update_schedule(id, active, next_datetime)` updates the schedule after each fire. `play(StoredEvent) -> StoredEvent` calculates the next occurrence from the event's stored fields and returns the event with `active` and `next_datetime` set.
