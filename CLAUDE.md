...
- **main.rs** — Bot entry point. Initializes teloxide REPL. All event and storage access goes through `EventProvider` (shared as `Arc<Mutex<EventProvider>>`). On startup calls `provider.reload()` then `schedule_first_event`. Handles incoming messages: for every message (any type), chat info is saved/updated via `provider.upsert_chat`. For text messages, the message is stored via `provider.insert_message` to obtain a `msg_id`. Then parses with `parser::parse`, sets `chat_id` and `msg_id` on the `EventInfo`, and calls `provider.insert(event)` which calculates datetime, persists to DB, reloads the next event, and returns the stored event plus the next schedulable event. After insertion calls `schedule_first_event` to reschedule. When the scheduled event fires: queries storage for ALL events at that timestamp via `provider.get_events_at(dt)`, sends all their messages through the channel as a `Vec<(ChatId, String)>`, then calls `provider.update_and_reload(events)` which recalculates each event and reloads the next one from DB. Then schedules the next event. The Telegram message-sending channel accepts `Vec<(ChatId, String)>` to support batching multiple simultaneous events. All responses use MarkdownV2 parse mode (escaped via `escape_markdown`).

- **state.rs** — `EventProvider` struct encapsulating `EventStorage`, a single `Option<EventInfo>` for the nearest active event, and an `AbortHandle` for the current scheduled task. Key methods: `upsert_chat(&ChatInfo)` delegates chat persistence to storage; `insert_message(user_id, chat_id, text)` constructs a `MessageInfo` and delegates to storage, returning the message ID; `get_next()` returns the nearest active event; `get_events_at(dt)` queries storage for all active events at a specific datetime; `update(event)` recalculates the event's next occurrence via `scheduler::calc_next_at` and saves to DB via `update_schedule`; `update_and_reload(events)` updates all given events then reloads the next event from DB; `insert(event)` calculates datetime via `scheduler::calc_next`, persists via `insert_event`, reloads the next event, and returns `(stored_event, next_event)`; `reload()` loads the single nearest active event from DB; `abort_current()` and `set_abort_handle()` manage the scheduled task handle.
