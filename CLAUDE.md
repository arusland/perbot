...
- **commands.rs** — Bot command handling. Defines the `Command` enum (`BotCommands`): `/help`, `/events`, `/today`, `/tomorrow`, `/month`, and admin-only `/exit` (`#[command(hide)]`). `CmdContext<'a>` bundles the shared deps (`bot`, `chat_id`, `provider`, `admin_id`, `is_admin`) every handler needs; `Command::handle(ctx)` dispatches to the private `handle_help`/`handle_events`/`handle_today`/`handle_tomorrow`/`handle_month`/`handle_exit`, each taking only `&CmdContext`. Replies use MarkdownV2 except `/help`, which is plain text.
