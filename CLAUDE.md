...
- **types.rs** — Shared types: `EventInfo` (rich parsed fields + DB-tracking fields, including `legacy: bool` for imported alerts and `snoozed: bool` for snooze-created events), `MessageInfo`, `ChatInfo`/`ChatType`, `TgMessage` (`chat_id`, `text`, and `snooze: bool` — when set, the sender attaches the snooze inline keyboard), `MessageSender`, plus the time enums (`TimeUnit`, `Repetition`, `Ordinal`, `MonthlyPattern`) and helpers `parse_days`, `unit_from_str`, `day_from_str`, `day_to_str`. `ChatType` implements `FromStr`. Weekday↔string mapping lives only here (`day_from_str`/`day_to_str`); other modules reuse it.
...
- **storage.rs** — `EventStorage` over rusqlite; public methods return `crate::error::Result`. Tables: `chats`, `messages`, `events` (`events.msg_id` is a NOT NULL FK to `messages`; `events.legacy` flags imported alerts and `events.snoozed` flags snooze-created events, both declared in the original `CREATE TABLE`). `open(path)` / `open_in_memory()`. Events: `insert_event`, `get_event`, `get_by_chat`, `get_active_events`, `get_active_by_chat`, `get_active_by_chat_on_date(chat_id, date)`, `get_active_by_chat_in_range(chat_id, start, end)` (end exclusive), `get_next_event`, `get_missed_events(now)`, `get_events_at(dt)`, `update_schedule`, `mark_inactive`, `delete`, `delete_inactive`. Chats: `upsert_chat`, `get_chat`, `get_all_chats`. Messages: `insert_message`.

- **state.rs** — `EventProvider`: a `Clone` handle wrapping `Arc<Mutex<EventProviderState>>` (storage + cached nearest `next_event`); all methods take `&self` and lock internally. `start(msg_tx)` sends missed events then spawns a 1s polling thread that fires due events and reschedules. Each due-event `TgMessage` is sent with `snooze: true` (the missed-events batch with `snooze: false`). Other methods wrap storage: `upsert_chat`, `insert_message`, `get_next_event`, `get_missed_events`, `get_event`, `get_active_by_chat`, `get_active_by_chat_on_date`, `get_active_by_chat_in_range`, `insert_event_and_get[_at]`, `insert_prebuilt_event` (inserts an already-scheduled event without re-running the scheduler — used by the legacy importer and by snooze), `update_at_and_reload`.

- **commands.rs** — Bot command handling. Defines the `Command` enum (`BotCommands`): `/help`, `/events`, `/today`, `/tomorrow`, `/week`, `/month`, and admin-only `/import <user_id>` and `/exit` (`#[command(hide)]`). `CmdContext<'a>` bundles the shared deps (`bot`, `chat_id`, `provider`, `admin_id`, `is_admin`, `pending_import`) every handler needs; `Command::handle(ctx)` dispatches `/help`/`/import`/`/exit` to `handle_help`/`handle_import`/`handle_exit` and the five list commands to the generic `handle_list(&ctx, kind)`. **All five list commands are paginated.** A private `ListKind` enum (`Events`/`Today`/`Tomorrow`/`Week`/`Month`) encapsulates each list's `tag()` (callback-data prefix `ev`/`td`/`tm`/`wk`/`mo`), `title()`, `empty()` message, and `fetch(provider, chat_id)` (the date ranges — today/tomorrow/this week Mon–Sun/this calendar month — are computed relative to "now", so paging recomputes them). `handle_list` renders page 0 via `telegram::format_page_at` (`LIST_PAGE_SIZE` events/page) and attaches an inline `◀ Prev`/`Next ▶` keyboard (private `list_keyboard`, callback data `<tag>:<page>`) when there is more than one page; on send failure it logs chat/event-count/length and warns the admin instead of bubbling up. Navigation is stateless: the public `handle_list_callback(bot, provider, q)` (driven from `main`'s callback-query branch) answers the callback, decodes `<tag>:<page>` via `ListKind::from_tag`, re-fetches that list's events, and edits the message in place to the requested page. `/import <user_id>` records the pending import target and prompts the admin to send the zip. The public `handle_import_zip(bot, provider, chat_id, target, file_id)` (called from `main` once the admin sends the zip) downloads it, runs `import::import_zip`, and replies with the summary + HTML report document. Replies use MarkdownV2 except `/help` and `/import`, which are plain text. **Snooze:** fired reminders carry an inline keyboard (public `snooze_keyboard`, options in `SNOOZE_OPTIONS`, callback data `sn:<minutes>`) offering 1 min/5 min/10 min/30 min/1 hour/8 hours/1 day. The public `handle_snooze_callback(bot, provider, q)` (driven from `main`'s callback-query branch for `sn:`-prefixed data) recovers the title from the reminder message text, then creates a new one-off event (`snoozed_event`: explicit-year, `snoozed=true`, `next = now + minutes`) at `now + <minutes>` via `insert_message` + `insert_prebuilt_event` — the original event is untouched, and the snoozed event goes inactive after firing.
...
- **main.rs** — Entry point + teloxide `Dispatcher` + wiring (env vars, message channel, `provider.start`, a `PendingImport` slot). At startup it clears stale commands from non-default scopes (`AllPrivateChats`/`AllGroupChats`/`AllChatAdministrators`) then `set_my_commands(Command::bot_commands())`. The dispatcher has two branches sharing deps via `dptree::deps!` (provider, admin_id, bot_username, pending_import): a `filter_message` branch (`message_handler`) and a `filter_callback_query` branch (`callback_handler`, which routes `sn:`-prefixed data to `commands::handle_snooze_callback` and everything else to `commands::handle_list_callback` for list pagination). The scheduled-message sender task attaches `commands::snooze_keyboard()` to any `TgMessage` whose `snooze` flag is set. `message_handler`: upserts chat, computes `is_admin`; if the admin sends a document while an import is pending, it calls `commands::handle_import_zip`, which downloads the zip, calls `import::import_zip`, and replies with a summary + the HTML report document. Otherwise, per text message it stores the message and `Command::parse` → builds a `CmdContext` → `cmd.handle(ctx)`; non-command text goes through `parser::parse` → `provider.insert_event_and_get`. Non-command replies use MarkdownV2 (escaped via `telegram::escape_markdown`).

- **telegram.rs** — `escape_markdown`; MarkdownV2 paginated event lists via `format_page(events, page, per_page, title, empty)` / `format_page_at(events, now, page, per_page, title, empty)` → `(text, total_pages)` (the `title` is a bare heading, e.g. `"Upcoming events"`, rendered as `*…:*` with a `(page x/y)` suffix only when there is more than one page; `empty` is the full no-events message). Helpers: `LIST_PAGE_SIZE`, `total_pages(len, per_page)`, and the private `write_event_row` (each row shows the absolute datetime plus a short relative time like `13 mins`, `1h`, `2d`, `1w` via the private `format_relative`). Also `extract_chat_info(chat)` → `ChatInfo`.
