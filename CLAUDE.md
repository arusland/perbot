...
- **main.rs** — Bot entry point. Initializes teloxide REPL. All event and storage access goes through `EventProvider` (shared as `Arc<Mutex<EventProvider>>`). On startup calls `provider.reload()` then `schedule_first_event`. Handles incoming messages: for every message (any type), chat info is saved/updated via `provider.upsert_chat`. For text messages, the message is stored via `provider.insert_message` to obtain a `msg_id`. Then parses with `parser::parse`, sets `chat_id` and `msg_id` on the `EventInfo`, and calls `provider.insert(event)` which calculates datetime, persists to DB, reloads top events, and returns the stored event plus the next schedulable event. After insertion calls `schedule_first_event` to reschedule. When the first event fires: calls `provider.update_and_get_next(event)` which recalculates, saves, updates the in-memory list, and returns the next event — then schedules it. All responses use MarkdownV2 parse mode (escaped via `escape_markdown`).

- **state.rs** — `EventProvider` struct encapsulating `EventStorage`, a sorted `Vec<EventInfo>` of the top nearest active events, and an `AbortHandle` for the current scheduled task. Key methods: `upsert_chat(&ChatInfo)` delegates chat persistence to storage; `insert_message(user_id, chat_id, text)` constructs a `MessageInfo` and delegates to storage, returning the message ID; `get_next()` returns the first (nearest) active event; `update_and_get_next(event)` recalculates the event's next occurrence via `scheduler::calc_next_at`, saves to DB via `update_schedule`, updates the sorted in-memory list (removes old entry, re-inserts sorted if still active, reloads from DB if list empties), and returns the first active event; `insert(event)` calculates datetime via `scheduler::calc_next`, persists via `insert_event`, reloads top events, and returns `(stored_event, next_event)`; `reload()` loads top events from DB; `abort_current()` and `set_abort_handle()` manage the scheduled task handle.
